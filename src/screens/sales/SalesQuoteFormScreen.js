...
export default function SalesQuoteFormScreen({ route, navigation }) {
  ...
  useEffect(() => {
    loadBusiness(businessId).then(b => {
      setBiz(b);
      if (quoteId) {
        const q = b?.salesQuotes?.find(q => q.id === quoteId);
        if (q) {
          setCustomer(b.customers?.find(c => c.id === q.customerId) || null);
          setDate(q.date || '');
  ...
  const handleSave = async () => {
    ...
    try {
      const existing = quoteId
        ? biz.salesQuotes?.find(q => q.id === quoteId)
        : null;
      const quote = {
      ...
      const updated = { ...biz };
      if (quoteId) {
        updated.salesQuotes = biz.salesQuotes.map(q =>
          q.id === quoteId ? quote : q
        );
      } else {
        updated.salesQuotes = [...(biz.salesQuotes || []), quote];
      }
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>
          {editing ? 'Edit Quote' : 'New Sales Quote'}
        </Text>
...
const styles = StyleSheet.create({
  ...
  stockText: { fontSize: 12, color: colors.primary, fontWeight: '600' },
  modalEmpty: {
