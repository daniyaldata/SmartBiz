...
export default function CustomerFormScreen({ route, navigation }) {
  ...
  useEffect(() => {
    loadBusiness(businessId).then(b => {
      setBiz(b);
      if (customerId) {
        const c = b?.customers?.find(x => x.id === customerId);
        if (c) {
          ...
          setOpeningBalance(c.openingBalance?.toString() || '');
        }
      }
    });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const updated = { ...biz };
      if (customerId) {
        updated.customers = biz.customers.map(c =>
          c.id === customerId ? customer : c
        );
      } else {
        updated.customers = [...(biz.customers || []), customer];
      }
    ...
    Alert.alert('Delete Customer', 'Are you sure?', [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          const updated = {
            ...biz,
            customers: biz.customers.filter(c => c.id !== customerId),
          };
          await saveBusiness(updated);
          navigation.goBack();
        },
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      >
        {[
          { label: 'Display name *', value: displayName, set: setDisplayName, placeholder: 'e.g. Ali Khan' },
