...
const STATUS = {
  ...
  uninvoiced: { bg: '#EFF6FF', text: '#1D4ED8', label: 'Un-invoiced' },
};

export default function SalesQuotesScreen({ route, navigation }) {
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Sales Quotes</Text>
        <View style={{ width: 22 }} />
      </View>
        ...
        ListEmptyComponent={
          <View style={styles.emptyBox}>
            ...
            <Text style={styles.emptySub}>
              Tap + to create a sales quote for a customer
            </Text>
          </View>
        }
        renderItem={({ item }) => {
          ...
          return (
            <TouchableOpacity
              style={styles.card}
              onPress={() =>
                navigation.navigate('SalesQuoteView', {
                  businessId,
                  quoteId: item.id,
                })
              }
              ...
              <View style={styles.cardTop}>
                <View style={{ flex: 1 }}>
                  <Text style={styles.cardNum}>
                    QUO-{item.number || item.id.slice(-4)}
                  </Text>
      ...
      <TouchableOpacity
        style={styles.fab}
        onPress={() =>
          navigation.navigate('SalesQuoteForm', { businessId })
        }
      >
...
const styles = StyleSheet.create({
  ...
  cardAmount: { fontSize: 16, fontWeight: '700', color: colors.primary },
  badge: {
