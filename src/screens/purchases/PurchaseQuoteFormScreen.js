...
export default function PurchaseQuoteFormScreen({ route, navigation }) {
  ...
  useEffect(() => {
    loadBusiness(businessId).then(b => {
      setBiz(b);
      if (quoteId) {
        const q = b?.purchaseQuotes?.find(q => q.id === quoteId);
        if (q) {
          setSupplier(b.suppliers?.find(s => s.id === q.supplierId) || null);
          setDate(q.date || '');
          setExpiryDate(q.expiryDate || '');
          setNotes(q.notes || '');
          setLines(q.lines?.length
            ? q.lines
            : [{ id: generateId(), description: '', qty: '1', rate: '' }]
          );
        }
  ...
  const total = lines.reduce(
    (sum, l) => sum + (parseFloat(l.qty) || 0) * (parseFloat(l.rate) || 0), 0
  );
  ...
  const handleSave = async () => {
    ...
    try {
      const existing = quoteId
        ? biz.purchaseQuotes?.find(q => q.id === quoteId)
        : null;
      const quote = {
        id: quoteId || generateId(),
        number: existing?.number ||
          String((biz.purchaseQuotes?.length || 0) + 1).padStart(4, '0'),
        ...
        expiryDate,
        notes,
        convertedToInvoiceId: existing?.convertedToInvoiceId || null,
        createdAt: existing?.createdAt || new Date().toISOString(),
      };
      const updated = { ...biz };
      if (quoteId) {
        updated.purchaseQuotes = biz.purchaseQuotes.map(q =>
          q.id === quoteId ? quote : q
        );
      } else {
        updated.purchaseQuotes = [...(biz.purchaseQuotes || []), quote];
      }
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>
          {editing ? 'Edit Quote' : 'New Purchase Quote'}
        </Text>
      ...
      >
        ...
        {editing && (
          <TouchableOpacity style={styles.deleteBtn} onPress={handleDelete}>
            ...
            <Text style={styles.deleteBtnText}>Delete Quote</Text>
          </TouchableOpacity>
        )}
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  label: {
    ...
    textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 7, marginTop: 16,
  },
  input: {
