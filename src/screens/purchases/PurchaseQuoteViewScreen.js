...
export default function PurchaseQuoteViewScreen({ route, navigation }) {
  ...
  const quote = biz.purchaseQuotes?.find(q => q.id === quoteId);
  ...
  const handleConvert = () => {
    if (isInvoiced) {
      Alert.alert(
        'Already invoiced',
        'This quote has already been converted to a purchase invoice.'
      );
      return;
    }
    Alert.alert(
      'Convert to Purchase Invoice',
      'This will create a new purchase invoice with the same details as this quote.',
      [
  ...
  const handleClone = async () => {
    ...
    const updated = {
      ...biz,
      purchaseQuotes: [...biz.purchaseQuotes, cloned],
    };
    await saveBusiness(updated);
    Alert.alert('Cloned!', `Quote PQUO-${cloned.number} created.`);
    navigation.replace('PurchaseQuoteView', {
      businessId,
      quoteId: cloned.id,
    });
  };
  ...
  const handleSharePdf = async () => {
    try {
      await generateQuotePdf(quote, biz, 'purchase');
    } catch (e) {
    ...
    Alert.alert('Delete Quote', 'This cannot be undone.', [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          const updated = {
            ...biz,
            purchaseQuotes: biz.purchaseQuotes.filter(q => q.id !== quoteId),
          };
          await saveBusiness(updated);
          navigation.goBack();
        },
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>
          PQUO-{quote.number || quoteId.slice(-4)}
        </Text>
        <TouchableOpacity
          onPress={() =>
            navigation.navigate('PurchaseQuoteForm', { businessId, quoteId })
          }
        >
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={[
          styles.statusBanner,
          { backgroundColor: isInvoiced ? '#DCFCE7' : '#FEF3C7' },
        ]}>
          <Ionicons
            name={isInvoiced ? 'checkmark-circle' : 'clipboard-outline'}
            size={20}
            color={isInvoiced ? '#16A34A' : '#D97706'}
          />
          <View style={{ flex: 1 }}>
            <Text style={[
              styles.statusText,
            ...
            {isInvoiced && (
              <Text style={[styles.statusSub, { color: '#16A34A' }]}>
        ...
        <View style={styles.card}>
          <View style={styles.detailRow}>
            ...
            <Text style={styles.detailValue}>{quote.supplierName}</Text>
          </View>
          <View style={styles.detailRow}>
        ...
        {!isInvoiced && (
          <TouchableOpacity style={styles.convertBtn} onPress={handleConvert}>
            ...
            <Text style={styles.convertBtnText}>Convert to Purchase Invoice</Text>
          </TouchableOpacity>
        )}
